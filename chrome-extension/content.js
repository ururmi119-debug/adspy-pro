...
function syncToServer(callback) {
  chrome.storage.local.get(['adradar_token', 'adsData', 'syncedIds'], function(r) {
    ...
    if (!pending.length) {
      console.log('[AdRadar] Nothing to sync — all', allAds.length, 'stored ads already marked synced.');
      if (callback) callback(true);
      return;
    }

    var batch = pending.slice(0, 50);
    console.log('[AdRadar] Syncing', batch.length, 'ads to server...');
    ...
    .then(function(result) {
      ...
      chrome.storage.local.set({
        ...
        lastSyncStatus: result.ok ? 'ok' : 'err',
        lastSyncTime: new Date().toISOString()
      }, function() {
        if (result.ok) {
          console.log('[AdRadar] Sync ok:', data.synced, 'synced,', data.failed, 'failed');
          showToast('Synced ' + data.synced + ' ads', data.failed ? (data.failed + ' failed') : '', 1600);
        } else {
          console.error('[AdRadar] Sync failed:', data);
          showToast('Sync failed', (data.error||'Server error').slice(0,60), 2200);
        }
        if (callback) callback(result.ok);
      });
    })
    .catch(function(err) {
      console.error('[AdRadar] Sync network error:', err);
      chrome.storage.local.set({ lastSyncStatus: 'err' }, function() {
        showToast('Sync failed', 'Network error — check connection', 2200);
...
function checkLoginStatus() {
  try {
    chrome.storage.local.get('adradar_token', function(r) {
      var loggedIn = !!r.adradar_token;
      var badge = document.getElementById('ap2-login-warn');
      if (!loggedIn) {
        if (!badge) {
          var wrap = document.getElementById('adspy-panel-v2');
          if (wrap) {
            var w = document.createElement('div');
            w.id = 'ap2-login-warn';
            w.style.cssText = 'background:rgba(239,68,68,0.15);border:1px solid rgba(239,68,68,0.4);color:#fca5a5;font-size:10px;font-weight:700;padding:6px 10px;border-radius:8px;margin-bottom:8px;cursor:pointer;';
            w.textContent = '🔒 Not logged in — data will NOT sync to your dashboard. Click here for instructions.';
            w.addEventListener('click', function(){
              showToast('Login required', 'Click the puzzle icon (🧩) in Chrome, open AdRadar, and log in', 3000);
            });
